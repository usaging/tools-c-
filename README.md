...
# deletetable.cpp，共享平台的公共功能模块，用于清理表中的数据。
...
# syncref.cpp，共享平台的公共功能模块，采用刷新的方法同步Oracle数据库之间的表。

它提供了两种同步策略：

不分批刷新：适用于数据量不大的情况，一次性完成同步。

分批刷新：适用于大数据量的情况，分批次进行同步。
...
# syncinc.cpp，共享平台的公共功能模块，采用增量的方法同步Oracle数据库之间的表。
...
struct st_route
{
    int srcport;    // 源端口（外网监听端口）
    char dstip[31]; // 目标主机的地址（内网服务地址）
    int dstport;    // 目标主机的端口（内网服务端口）
